...
项目页按 `product.json` 的实际字段动态显示。所有字段的值统一为字符串，支持 Markdown 标题、列表、加粗、链接、代码块和表格。五个基础字段使用中文标题，新增字段直接以字段名显示。点击编辑按钮进入表单，各字段显示原始 Markdown 文本；「增加字段」仅在编辑窗口中提供。保存时保留原文的缩进和换行。五个基础字段的校验仍然保留，扩展字段不会因编辑基础信息而丢失。

产品文件和目标文件的内容字段均使用 Markdown 字符串。目标的完成条件、任务约束和进展使用多行原文编辑，详情页展示渲染结果，扩展字段也会展示和保留。目标 `status` 仍使用 `open` / `completed` 管理完成状态。

旧文件在读取时兼容转换：字符串列表变为 Markdown 列表，其他非字符串值保存在 JSON 代码块中。读取不改写文件，用户明确保存后才写入统一的字符串格式。新写入的字段不接受数组、对象等非字符串值。
